...
func Exec(ctx context.Context, args Args) error {
	...
	if err := writeEnvToFile("GITHUB_APP_TOKEN", token); err != nil {
		return err
	}
	
	if err := writeEnvToFile("GITHUB_APP_INSTALLATION_ID", fmt.Sprintf("%d", installationID)); err != nil {
		return err
	}
	
	if err := writeEnvToFile("GITHUB_APP_SLUG", appSlug); err != nil {
		return err
	}
...
// writeEnvToFile writes environment variables to the Harness output file
